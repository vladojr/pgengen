...
public class Sql implements ResultSet, AutoCloseable {

	private final PreparedStatement stmt;
	private final ResultSet rs;

	public Sql(final PreparedStatement stmt) throws SQLException {
		this.stmt = stmt;
		this.rs = stmt.executeQuery();
	}

	public static String paramList(final int length) {
		if (length == 0) return "";
		if (length == 1) return "?";
		if (length == 2) return "?,?";
		if (length == 3) return "?,?,?";
	...
	public static void execute(final Connection conn, final String query, final Object... params) throws SQLException {
		final PreparedStatement s = conn.prepareStatement(query);
		try {
			if (params != null && params.length > 0) {
				for (int x = 0; x < params.length; x++) {
					s.setObject(x + 1, params[x]);
				}
			}
			s.execute();
		} finally {
	...
	public static Sql from(final Connection conn, final String query, final Object... params) throws SQLException {
		final PreparedStatement s = conn.prepareStatement(query);
		try {
			if (params != null && params.length > 0) {
				for (int x = 0; x < params.length; x++) {
